# Yuchen Panel V0.7.7.1 Node Diagnosis Patch
...
1. `deploy/install.sh`
   - 新增 `YC_INSTALL_MODE=auto|fast|docker`
   - 检测到 `bin/yuchen-panel-api-linux-amd64`、`bin/yuchen-agent-linux-amd64`、`frontend/dist/index.html` 时自动进入 fast 模式
...
```text
dist-release/yuchen-panel-v0.7.7.1-node-diagnosis-center.zip
...
```bash
YC_INSTALL_MODE=fast bash deploy/install.sh
```
...
```bash
YC_INSTALL_MODE=docker bash deploy/install.sh
```
