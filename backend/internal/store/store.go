...
import (
	...
	"yuchen-panel/backend/internal/security"
)
...
func (s *Store) seedDefaultAdmin() error {
	now := time.Now()
	username := getenv("YC_ADMIN_USERNAME", "admin")
	password := getenv("YC_ADMIN_PASSWORD", "admin123")
...
func (s *Store) seedLocalServer() error {
	now := time.Now()
	serverID := NewID("srv")
	ip := getenv("YC_LOCAL_SERVER_IP", "127.0.0.1")
	host := getenv("YC_LOCAL_SERVER_HOST", ip)
	name := getenv("YC_LOCAL_SERVER_NAME", "本机服务器")
	region := getenv("YC_LOCAL_SERVER_REGION", "Local")
	provider := getenv("YC_LOCAL_SERVER_PROVIDER", "Self-hosted")
...
func (s *Store) ensureSingleModeLocalServer() (bool, error) {
	...
	localIP := getenv("YC_LOCAL_SERVER_IP", "127.0.0.1")
	localHost := getenv("YC_LOCAL_SERVER_HOST", localIP)
	localName := getenv("YC_LOCAL_SERVER_NAME", "本机服务器")
	localRegion := getenv("YC_LOCAL_SERVER_REGION", "Local")
	localProvider := getenv("YC_LOCAL_SERVER_PROVIDER", "Self-hosted")
