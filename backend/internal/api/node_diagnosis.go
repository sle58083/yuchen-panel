...
func (r *Router) buildNodeDiagnosis() nodeDiagnosisResponse {
	panelInfo := readKVFile("/etc/yuchen-panel/panel.info")
	envInfo := readKVFile("/opt/yuchen-panel/.env")
	panelPort := firstNonEmptyDiag(panelInfo["PORT"], envInfo["YC_PANEL_PORT"])
	webBasePath := firstNonEmptyDiag(panelInfo["WEB_BASE_PATH"], envInfo["YC_WEB_BASE_PATH"])
	installMode := firstNonEmptyDiag(panelInfo["INSTALL_MODE"], envInfo["YC_INSTALL_MODE"])
	if installMode == "" {
		installMode = "unknown"
	}
	...
	runtime = append(runtime, serviceCheck("yuchen-panel-api", "API 服务"))
	runtime = append(runtime, serviceCheck("yuchen-agent", "Agent 服务"))
	...
	if canTCPDial("127.0.0.1", 8088, 1200*time.Millisecond) {
		...
		runtime = append(runtime, diagnosisItem{Key: "api_port", Label: "API 本机端口", Status: "fail", Message: "127.0.0.1:8088 无法连接，请检查 yuchen-panel-api 服务。"})
		recommendations = append(recommendations, "执行 systemctl status yuchen-panel-api --no-pager -l 查看 API 服务状态。")
	}
	...
	if panelPort != "" {
		...
		runtime = append(runtime, diagnosisItem{Key: "panel_port_missing", Label: "面板入口端口", Status: "warn", Message: "未在 /etc/yuchen-panel/panel.info 中读取到 PORT。"})
	}

	nginxConf, err := os.ReadFile("/etc/nginx/conf.d/yuchen-panel.conf")
	if err != nil {
		config = append(config, diagnosisItem{Key: "nginx_conf", Label: "Nginx 配置文件", Status: "warn", Message: "未读取到 /etc/nginx/conf.d/yuchen-panel.conf。", Detail: err.Error()})
		...
		if len(missing) == 0 {
			...
			recommendations = append(recommendations, "重新运行 V0.7.7.1 安装脚本，或修复 /etc/nginx/conf.d/yuchen-panel.conf。")
		}
	...
	xrayConfigPath := firstExistingFile("/etc/yuchen-panel/xray/config.json", "/usr/local/etc/xray/config.json")
	if xrayConfigPath != "" {
...
func renderNodeDiagnosisReport(resp nodeDiagnosisResponse) string {
	...
	b.WriteString("Yuchen Panel 节点诊断报告\n")
	b.WriteString("========================\n")
...
func detectPublicIPv6() string {
	...
	req.Header.Set("User-Agent", "Yuchen-Panel/"+panelVersion)
