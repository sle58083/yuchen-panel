...
func (r *Router) updateTaskCreate(w http.ResponseWriter, req *http.Request) {
	...
	task := upgradeTaskState{
		...
		WorkDir:        filepath.Join("/root/yuchen-panel-upgrades", id),
...
func managedUpgradeSupported() bool {
	if _, err := os.Stat("/etc/yuchen-panel/panel.info"); err != nil {
		return false
	}
...
func startUpgradeRunner(taskID, script string) error {
	unit := "yuchen-panel-upgrade-" + strings.ReplaceAll(taskID, "_", "-")
...
echo "========== Yuchen Panel Upgrade Task $TASK_ID =========="
echo "Start: $(date)"
...
mkdir -p "$BACKUP_DIR"
[ -d /opt/yuchen-panel/data ] && tar -czf "$BACKUP_DIR/data.tar.gz" -C /opt/yuchen-panel data || true
[ -d /etc/yuchen-panel ] && tar -czf "$BACKUP_DIR/etc-yuchen-panel.tar.gz" -C /etc yuchen-panel || true
[ -f /etc/nginx/conf.d/yuchen-panel.conf ] && cp -a /etc/nginx/conf.d/yuchen-panel.conf "$BACKUP_DIR/yuchen-panel.nginx.conf" || true
[ -f /opt/yuchen-panel/.env ] && cp -a /opt/yuchen-panel/.env "$BACKUP_DIR/dot-env" || true
...
YC_UPDATE_MANIFEST_URL="$MANIFEST_URL" YC_INSTALL_MODE=fast bash deploy/install.sh || fail 33 installing "安装脚本执行失败"

write_status restarting restarting "安装脚本完成，正在等待 API 恢复并校验版本"
systemctl restart yuchen-panel-api nginx yuchen-agent >/dev/null 2>&1 || true
...
func upgradePrechecks() []map[string]any {
	...
	add("远程版本清单", updateManifestURL() != "", firstNonEmpty(updateManifestURL(), "未配置 YC_UPDATE_MANIFEST_URL"))
