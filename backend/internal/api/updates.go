...
func updateManifestURL() string {
	return strings.TrimSpace(getenv("YC_UPDATE_MANIFEST_URL", ""))
}
...
func (r *Router) updateStatus(w http.ResponseWriter, req *http.Request) {
	...
	note := "未配置远程版本清单。请先把代码仓库和 version.json 发布好，再通过环境变量 YC_UPDATE_MANIFEST_URL 配置正式地址。"
	if configured {
...
func (r *Router) updateCheck(w http.ResponseWriter, req *http.Request) {
	...
	if manifestURL == "" {
		writeJSON(w, http.StatusOK, map[string]any{
			...
			"current_version": panelVersion,
			"manifest_url":    "",
			"message":         "暂未配置远程版本清单。请先上传代码仓库并发布 version.json，再设置 YC_UPDATE_MANIFEST_URL。",
		})
		return
	}
...
func (r *Router) updatePanelCommand(w http.ResponseWriter, req *http.Request) {
	...
	manifestURL := updateManifestURL()
	if manifestURL == "" {
		writeJSON(w, http.StatusOK, map[string]any{"ok": false, "message": "无法生成升级命令：远程版本清单未配置。请先发布 version.json 并设置 YC_UPDATE_MANIFEST_URL。"})
		return
	}
	manifest, err := fetchUpdateManifest(req.Context(), manifestURL)
	if err != nil {
	...
	command := fmt.Sprintf("cd /root && \\\nmkdir -p /root/yuchen-panel-upgrades && \\\ncd /root/yuchen-panel-upgrades && \\\ncurl -L --fail -o %s %s && \\\n%sunzip -o %s && \\\ncd %s && \\\nbash deploy/install.sh 2>&1 | tee /root/yuchen-panel-upgrade.log", shellEscape(pkg), shellEscape(manifest.DownloadURL), shaLine, shellEscape(pkg), shellEscape(dir))
	writeJSON(w, http.StatusOK, map[string]any{
		"ok":              true,
		"current_version": panelVersion,
