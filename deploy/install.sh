...
YC_INSTALL_MODE=${YC_INSTALL_MODE:-auto}   # auto | fast | docker
...
DEFAULT_UPDATE_MANIFEST_URL="https://raw.githubusercontent.com/sle58083/yuchen-panel/main/version.json"
...
has_fast_assets() {
  [[ -f "$SRC_DIR/bin/yuchen-panel-api-linux-amd64" || -f "$SRC_DIR/bin/yuchen-panel-api" ]] || return 1
  [[ -f "$SRC_DIR/bin/yuchen-agent-linux-amd64" || -f "$SRC_DIR/bin/yuchen-agent" ]] || return 1
  [[ -f "$SRC_DIR/frontend/dist/index.html" || -f "$SRC_DIR/web/index.html" ]] || return 1
  return 0
}

selected_install_mode() {
  if [[ "$YC_INSTALL_MODE" == "fast" ]]; then
    echo "fast"
    return
  fi
  if [[ "$YC_INSTALL_MODE" == "docker" ]]; then
    echo "docker"
    return
  fi
...
installer_backup_existing() {
  ...
  for item in \
    ...
    "/etc/systemd/system/xray.service.d/99-yuchen-panel.conf"; do
    if [[ -e "$item" ]]; then
      has_existing="true"
      break
    fi
  ...
  if [[ "$has_existing" != "true" ]]; then
    echo "No existing Yuchen Panel data/config found, skip pre-install backup."
    return 0
  fi
  ...
  ts="$(date +%Y%m%d-%H%M%S)"
  backup="$backup_dir/yuchen-panel-backup-${ts}.tar.gz"
  mkdir -p "$backup_dir"
  tmp="$(mktemp -d)"
  root="$tmp/root"
  mkdir -p "$root"
...
cleanup_old_runtime() {
  step "Cleaning old Yuchen Panel runtime"
  systemctl stop yuchen-panel-api 2>/dev/null || true
  systemctl stop yuchen-agent 2>/dev/null || true
  if command -v docker >/dev/null 2>&1; then
    docker rm -f yuchen-panel-api yuchen-panel-frontend 2>/dev/null || true
    docker ps -aq --filter "name=yuchen-panel" | xargs -r docker rm -f 2>/dev/null || true
  fi
  rm -f /etc/nginx/conf.d/yuchen-panel.conf 2>/dev/null || true
}
...
DATABASE=JSON (${APP_DIR}/data/yuchen-panel.json)
Database=JSON (${APP_DIR}/data/yuchen-panel.json)
...
write_host_nginx_docker() {
  ...
  cat > /etc/nginx/conf.d/yuchen-panel.conf <<EOF_NGINX
server {
    listen ${PANEL_PORT};
    server_name _;
...
install_cli() {
  step "Installing yuchen-panel CLI"
  install -m 0755 "$APP_DIR/scripts/yuchen-panel" /usr/local/bin/yuchen-panel
}
...
post_install_self_check() {
  ...
  if command -v yuchen-panel >/dev/null 2>&1; then
    yuchen-panel doctor || true
  else
    echo "yuchen-panel CLI not found, skip doctor check."
  fi
}

print_result() {
  echo
  echo "Yuchen Panel installed successfully"
  echo
  ...
  echo "Database:    JSON (${APP_DIR}/data/yuchen-panel.json)"
...
wait_api() {
  ...
  if [[ "${INSTALL_MODE}" == "fast" ]]; then
    echo "Check logs: journalctl -u yuchen-panel-api -n 120 --no-pager"
  else
    echo "Check logs: cd ${APP_DIR} && ${COMPOSE} logs --tail=120 yuchen-panel-api"
  fi
  exit 1
}
...
install_local_agent() {
  ...
  SERVER_PICK=$(APP_DIR="$APP_DIR" YC_LOCAL_SERVER_IP="$PUBLIC_IP" YC_LOCAL_SERVER_HOST="$LOCAL_HOST" python3 - <<'PY'
import json, os
p=os.path.join(os.environ.get('APP_DIR','/opt/yuchen-panel'),'data','yuchen-panel.json')
try:
...
local_ip=os.environ.get('YC_LOCAL_SERVER_IP','')
local_host=os.environ.get('YC_LOCAL_SERVER_HOST','')
def score(s):
    v=0
...
PY
  ...
  if [[ -z "$SERVER_ID" || -z "$AGENT_TOKEN" ]]; then
    ...
    INSTALL_XRAY="$INSTALL_XRAY" SETUP_XRAY_SERVICE="$SETUP_XRAY_SERVICE" YC_FORCE_INSTALL_XRAY="$YC_FORCE_INSTALL_XRAY" YC_SKIP_XRAY_INSTALL="$YC_SKIP_XRAY_INSTALL" APPLY_CONFIG=true PANEL_BASE="http://127.0.0.1:${API_PORT}" SERVER_ID="$SERVER_ID" AGENT_TOKEN="$AGENT_TOKEN" ./deploy/agent-install.sh
  fi
}
...
WEB_PORT=${WEB_PORT}
WEB_BASE_PATH=${WEB_BASE_PATH}
YC_JWT_SECRET=${JWT_SECRET}
YC_AGENT_SHARED_SECRET=${AGENT_SECRET}
YC_ADMIN_USERNAME=${ADMIN_USERNAME}
YC_ADMIN_PASSWORD=${ADMIN_PASSWORD}
YC_DB_PATH=${APP_DIR}/data/yuchen-panel.json
YC_API_ADDR=127.0.0.1:${API_PORT}
YC_LOCAL_SERVER_IP=${PUBLIC_IP}
YC_LOCAL_SERVER_HOST=${LOCAL_HOST}
YC_LOCAL_SERVER_NAME=本机服务器
YC_LOCAL_SERVER_REGION=Local
YC_LOCAL_SERVER_PROVIDER=Self-hosted
YC_UPDATE_MANIFEST_URL=${MANIFEST_URL_TO_WRITE}
EOF_ENV

  if [[ -n "$old_env" && -f "$old_env" ]]; then
    grep -vE '^(API_PORT|WEB_PORT|WEB_BASE_PATH|YC_JWT_SECRET|YC_AGENT_SHARED_SECRET|YC_ADMIN_USERNAME|YC_ADMIN_PASSWORD|YC_DB_PATH|YC_API_ADDR|YC_LOCAL_SERVER_IP|YC_LOCAL_SERVER_HOST|YC_LOCAL_SERVER_NAME|YC_LOCAL_SERVER_REGION|YC_LOCAL_SERVER_PROVIDER|YC_UPDATE_MANIFEST_URL)=' "$old_env" >> .env || true
    rm -f "$old_env"
  fi
  chmod 600 .env
}
...
start_fast_runtime() {
  ...
  local api_bin="$APP_DIR/bin/yuchen-panel-api-linux-amd64"
  if [[ ! -f "$api_bin" ]]; then
    api_bin="$APP_DIR/bin/yuchen-panel-api"
  fi
  if [[ ! -f "$api_bin" ]]; then
    echo "ERROR: fast install selected but API binary is missing."
    echo "Expected: $APP_DIR/bin/yuchen-panel-api-linux-amd64"
    exit 1
  fi
  chmod +x "$api_bin"
  cat > /etc/systemd/system/yuchen-panel-api.service <<EOF_SERVICE
[Unit]
Description=Yuchen Panel API
...
EOF_SERVICE
  ...
  systemctl enable yuchen-panel-api >/dev/null 2>&1 || true
...
main() {
  echo "Yuchen Panel V${VERSION} product installer"
  echo "Target: ${APP_DIR}"
  ...
  PUBLIC_IP=${YC_LOCAL_SERVER_IP:-$(public_ip)}
  LOCAL_HOST=${YC_LOCAL_SERVER_HOST:-$PUBLIC_IP}
  ...
  EXISTING_MANIFEST_URL=$(env_file_value YC_UPDATE_MANIFEST_URL)
  ...
  ADMIN_USERNAME=${YC_ADMIN_USERNAME:-${EXISTING_USERNAME:-$(random_string 10)}}
  ADMIN_PASSWORD=${YC_ADMIN_PASSWORD:-${EXISTING_PASSWORD:-$(random_string 12)}}
  ADMIN_PASSWORD_DISPLAY="$ADMIN_PASSWORD"
  JWT_SECRET=$(random_string 64)
  AGENT_SECRET=${EXISTING_AGENT_SECRET:-$(random_string 64)}
  MANIFEST_URL_TO_WRITE=${YC_UPDATE_MANIFEST_URL:-${EXISTING_MANIFEST_URL:-$DEFAULT_UPDATE_MANIFEST_URL}}
  ...
  if [[ -f data/yuchen-panel.json && -s data/yuchen-panel.json && "$FRESH_INSTALL" != "true" ]]; then
