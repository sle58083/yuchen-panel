    ...
    <div class="modal-card share-modal-card">
      ...
      <div v-else class="share-modal-body">
        <div class="share-config-box">
          <div class="share-config-head"><strong>{{ shareHeading() }}</strong><div class="inline-actions"><template v-if="shareTab === 'clash' && shareClient"><button class="btn secondary" @click="copyClashRemoteUrl">复制 Clash Verge 订阅链接</button><a class="btn secondary" :href="clashDownloadUrl(shareClient)" download="yuchen-clash.yaml">下载 clash.yaml</a></template><button class="btn" @click="copyAny(shareText, shareCopyLabel())">复制{{ shareCopyLabel() }}</button></div></div>
