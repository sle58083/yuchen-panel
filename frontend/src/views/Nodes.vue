...
watch(() => form.value.transport, () => {
  ...
  if (form.value.transport === 'ws' && !form.value.path) form.value.path = '/yuchen'
  if (form.value.transport === 'grpc' && !form.value.path) form.value.path = 'yuchen'
})
...
function applySocks5(generate = true) {
  ...
  form.value.socks_username = form.value.socks_username || 'yuchen'
...
function editNode(n:any) {
  ...
  form.value = {
    ...
    sni: n.sni || '',
    path: n.transport === 'tcp' ? '' : (n.path || (n.transport === 'ws' ? '/yuchen' : 'yuchen')),
  ...
  <div v-if="showEditor" class="modal-mask" @click.self="showEditor = false">
    ...
    <div v-if="form.protocol === 'socks'" class="reality-box">
      ...
      <div class="form node-form clean-form reality-form">
        <label class="field"><span>SOCKS5 账号</span><input v-model="form.socks_username" placeholder="yuchen" /></label>
