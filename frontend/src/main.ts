...
const previousFrontendVersion = localStorage.getItem('yc_frontend_version')
if (previousFrontendVersion && previousFrontendVersion !== APP_VERSION) {
  localStorage.removeItem('yc_token')
}
localStorage.setItem('yc_frontend_version', APP_VERSION)
...
router.beforeEach((to) => {
  if (to.path !== '/login' && !localStorage.getItem('yc_token')) return '/login'
})
