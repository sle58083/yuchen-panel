...
func main() {
	cfg := Config{
		PanelBase:     strings.TrimRight(getenv("YC_PANEL_BASE", "http://127.0.0.1:8088"), "/"),
		ServerID:      getenv("YC_SERVER_ID", ""),
		AgentToken:    getenv("YC_AGENT_TOKEN", ""),
		ConfigPath:    getenv("YC_XRAY_CONFIG", "/etc/yuchen-panel/xray/config.json"),
		TestCommand:   getenv("YC_XRAY_TEST_CMD", "xray run -test -config {config}"),
		ReloadCommand: getenv("YC_XRAY_RELOAD_CMD", "systemctl restart xray"),
		Interval:      time.Duration(getenvInt("YC_AGENT_INTERVAL_SECONDS", 30)) * time.Second,
		ApplyConfig:   getenvBool("YC_APPLY_CONFIG", true),
	}
	if cfg.ServerID == "" || cfg.AgentToken == "" {
		log.Fatal("YC_SERVER_ID and YC_AGENT_TOKEN are required")
	}
	hostname, _ := os.Hostname()
	lastMessage := "agent started"
	log.Printf("Yuchen Agent %s started, panel=%s, server_id=%s, config=%s", version, cfg.PanelBase, cfg.ServerID, cfg.ConfigPath)
	for {
