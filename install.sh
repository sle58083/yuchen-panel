...
REPO_OWNER="${YC_REPO_OWNER:-sle58083}"
REPO_NAME="${YC_REPO_NAME:-yuchen-panel}"
REPO_BRANCH="${YC_REPO_BRANCH:-main}"
INSTALL_ROOT="${YC_INSTALL_ROOT:-/root}"
MANIFEST_URL="${YC_UPDATE_MANIFEST_URL:-https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/${REPO_BRANCH}/version.json}"
WORK_DIR="$(mktemp -d)"
START_TS="$(date +%s)"
APT_UPDATED=false
...
panel_info_value() {
  local key="$1"
  if [ -r /etc/yuchen-panel/panel.info ]; then
    awk -F= -v k="${key}" '$1==k {print $2; exit}' /etc/yuchen-panel/panel.info 2>/dev/null || true
  fi
}
...
run_deploy_install() {
  ...
  log "执行部署脚本"
  export YC_UPDATE_MANIFEST_URL="${MANIFEST_URL}"
